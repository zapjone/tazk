...
object TazkMapFunctions {
  ...
  def rowToDocumentMapper(schema: StructType, convertFieldNameFun: String => String = x => x): (Row) => BsonDocument = {
    ...
    val mappers = schema.fields.map({ field =>
      if (field.dataType == NullType) {
        (data: Any, document: BsonDocument) => document.append(convertFieldNameFun(field.name), new BsonNull())
      } else {
        val mapper = wrappedDataTypeToBsonValueMapper(field.dataType)
        (data: Any, document: BsonDocument) => if (data != null) document.append(convertFieldNameFun(field.name), mapper(data))
      }
    })
