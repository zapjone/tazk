  ...
  private def initializeLogging(isInterpreter: Boolean, silent: Boolean): Unit = {
    ...
    if (Logging.isLog4j12()) {
      ...
      if (!log4j12Initialized) {
        Logging.defaultTazkLog4jConfig = true
        val defaultLogProps = "tazk/log4j-defaults.properties"
        Option(Utils.getTazkClassLoader.getResource(defaultLogProps)) match {
          case Some(url) =>
            PropertyConfigurator.configure(url)
            if (!silent) {
              System.err.println(s"Using Tazk's default log4j profile: $defaultLogProps")
            }
          case None =>
            System.err.println(s"Tazk was unable to load $defaultLogProps")
        }
      }
...
private[tazk] object Logging {
  @volatile private var initialized = false
  @volatile private var defaultRootLevel: Level = null
  @volatile private var defaultTazkLog4jConfig = false
  ...
  def uninitialize(): Unit = initLock.synchronized {
    if (isLog4j12()) {
      if (defaultTazkLog4jConfig) {
        defaultTazkLog4jConfig = false
